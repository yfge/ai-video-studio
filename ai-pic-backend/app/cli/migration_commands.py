...
def upgrade(revision: str, backup: bool, validate: bool):
    ...
    try:
        ...
        click.echo(f"目标版本: {status['head_revision'] if revision == 'head' else revision}")
        ...
        # 确认升级
        if not click.confirm(f"确认升级数据库吗？"):
            click.echo("操作已取消")
            return
        
        # 执行升级
        migration_manager.upgrade(revision)
        click.echo(f"✅ 数据库升级成功")
