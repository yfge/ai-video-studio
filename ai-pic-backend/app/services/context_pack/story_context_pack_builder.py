...
def build_episode_context_pack(
    *,
    ...
    """Build a StoryContextPack with Episode temporary characters included.

    Budget allocation strategy:
    - 50% of max_character_cards for Story main characters (importance >= 3)
    - 50% for Episode temporary characters (ordered by importance)
    - Remaining slots filled with Story supporting characters (importance < 3)

    Args:
        db: Database session
        story_id: Story ID
        episode_id: Episode ID to include temporary characters
        story_snapshot: Merged story data snapshot
        continuity_ledger: Continuity tracking data
        generation_params: Optional generation parameters
        budget: Optional budget configuration

    Returns:
        Dictionary representing the context pack with Story and Episode characters
    """
    budget = budget or ContextPackBudget()
    trims: list[str] = []

    total_slots = budget.max_character_cards
    story_main_slots = max(1, total_slots // 2)  # 50% for Story main characters
    episode_slots = max(1, total_slots // 2)  # 50% for Episode characters

    # Fetch Story main characters (importance >= 3)
    story_main_rows = (
        _not_deleted(db.query(StoryCharacter), StoryCharacter)
        .join(
            VirtualIP,
    ...
    # Fetch Episode temporary characters
    episode_char_rows = (
        _not_deleted(db.query(EpisodeCharacter), EpisodeCharacter)
        .join(
            VirtualIP,
            (EpisodeCharacter.virtual_ip_id == VirtualIP.id)
            & VirtualIP.is_deleted.is_(False),
        )
    ...
    # Build character cards for Story main characters
    cards: list[CharacterCard] = []
    for sc in story_main_rows:
        vip = getattr(sc, "virtual_ip", None)
        if not vip:
            continue
        cards.append(
    ...
    # Build character cards for Episode temporary characters
    for ec in episode_char_rows:
        vip = getattr(ec, "virtual_ip", None)
        if not vip:
            continue

        # Resolve voice config: use override if provided, else VirtualIP default
        voice_config = ec.voice_config_override
        ...
        # Build appearance description: merge VirtualIP + override
        appearance = vip.style_prompt or ""
        if ec.appearance_override:
            appearance = f"{appearance}\n{ec.appearance_override}".strip()

        # Use character_name if set, else VirtualIP name
        ...
        cards.append(
            CharacterCard(
                ...
                description=truncate_text(
                    ec.personality or vip.description, budget.max_field_chars
                ),
                background_story=truncate_text(
                    ec.background or vip.background_story, budget.max_field_chars
                ),
                biography=truncate_text(vip.biography, budget.max_field_chars),
                style_prompt=truncate_text(appearance, budget.max_field_chars),
                voice_config=voice_config,
            )
        )

    # Fill remaining slots with Story supporting characters (importance < 3)
    ...
    if remaining_slots > 0:
        ...
        for sc in story_supporting_rows:
            vip = getattr(sc, "virtual_ip", None)
            if not vip:
                continue
            cards.append(
                CharacterCard(
                    ...
                    biography=truncate_text(vip.biography, budget.max_field_chars),
                    style_prompt=truncate_text(
                        vip.style_prompt, budget.max_field_chars
                    ),
                    voice_config=(
                        vip.voice_config
                        if isinstance(vip.voice_config, dict) and vip.voice_config
    ...
    # Recent episode summaries (same logic as build_story_context_pack)
    recent_rows = (
        _not_deleted(db.query(Episode), Episode)
        .filter(Episode.story_id == story_id)
        .order_by(Episode.episode_number.asc())
    ...
    # Build generation params (same logic as build_story_context_pack)
    if not isinstance(generation_params, dict):
        generation_params = (
            story_snapshot.get("generation_params")
            if isinstance(story_snapshot.get("generation_params"), dict)
