...
knowledge_evidence 必须逐项覆盖 required_knowledge_evidence_keys。每个 key 对应同一条 knowledge_grant，value 必须是正文中一条连续、不含省略拼接的获知句：该 Canon 角色姓名必须直接建立“得知/获悉/确认/听见/看见/收到”或“告诉/告知/通知/透露/说明/宣布”等获知关系；source event 事实自身含有“发现”等动词时不得删改。
每条 knowledge_evidence 必须完整、连续地包含在对应 source_event_id 的 evidence 某个逐字片段中；knowledge_evidence 自身不得跨“……”拼接。不得用角色在别处出现的姓名拼接他人对话，不得以事件概括代替角色实际获知。
只输出严格 JSON：
