...
chapter_contract.location_transitions 是本章允许发生的全部地点移动；为空时所有人物和物件必须保持 current_state 地点，“准备转移”不等于开始移动、离开当前位置或抵达目标地点；非空时只能逐项发生清单中的移动。
...
{KNOWLEDGE_EVIDENCE_RULE}
逐项必备获知句合同（source_event_text 是该 grant 必须表达的当前事件事实）：{json_prompt_payload(knowledge_sentence_prefixes(context_pack))}
chapter_contract.location_transitions 是本章允许发生的全部地点移动；为空时删除全部开始移动、离开当前位置或抵达目标地点情节并保持 current_state 地点；非空时只能逐项发生清单中的移动。
