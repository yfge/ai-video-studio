...
class User(SoftDeleteBusinessMixin, Base):
    ...
    approved_by_user_id = Column(Integer, ForeignKey("users.id"), nullable=True, comment="审批人ID")
    
    # 邮箱验证相关
    email_verified = Column(Boolean, default=False, comment="邮箱是否已验证")
    activation_token = Column(String(255), nullable=True, comment="激活令牌")
    activation_token_expires = Column(DateTime(timezone=True), nullable=True, comment="激活令牌过期时间")
    
    # 登录相关
    last_login_at = Column(DateTime(timezone=True), nullable=True, comment="最后登录时间")
    failed_login_attempts = Column(Integer, default=0, comment="失败登录次数")
    account_locked_until = Column(DateTime(timezone=True), nullable=True, comment="账户锁定到期时间")
...
class UserAuditLog(SoftDeleteBusinessMixin, Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, comment="被操作用户ID")
    admin_user_id = Column(Integer, ForeignKey("users.id"), nullable=True, comment="操作管理员ID")
