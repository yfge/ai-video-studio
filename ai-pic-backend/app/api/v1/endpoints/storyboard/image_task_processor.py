...
from .image_task_refs import build_frame_references, load_image_ref_context
...
def _generate_frame_image(
    ...
    *,
    ...
    async def _gen(p, refs):
        try:
            ...
            return await generate_storyboard_image_urls(
                prompt=p,
                refs=[_abs_url(u) for u in refs if u],
                model=model,
                generation_profile=generation_profile,
                count=count,
                size=size,
    ...
    async def _persist(
        url, fidx, provider, mdl, *, keyframe_role="single", variant_index=None
    ):
        try:
            meta = {
                "script_id": script_id,
