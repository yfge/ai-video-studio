...
def list_users(
    ...
    status_filter: Optional[str] = Query(None, description="状态筛选: pending, approved, suspended, locked"),
    role_filter: Optional[str] = Query(None, description="角色筛选: admin, superuser, user"),
...
def suspend_user(
    user_id: int,
    duration_hours: Optional[int] = Query(None, description="暂停时长（小时），不设置则永久暂停"),
