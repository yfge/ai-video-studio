...
class EpisodeCharacterBase(BaseModel):
    ...
    character_name: Optional[str] = Field(None, max_length=100, description="Character name (overrides VirtualIP.name)")
    role_type: Optional[str] = Field("temporary", max_length=50, description="Role type: temporary/guest/extra")
    importance: Optional[int] = Field(1, ge=1, le=5, description="Importance level 1-5")
    personality: Optional[str] = Field(None, description="Personality (overrides VirtualIP)")
    background: Optional[str] = Field(None, description="Background (overrides VirtualIP)")
    appearance_override: Optional[str] = Field(None, description="Appearance supplement to VirtualIP.style_prompt")
    voice_config_override: Optional[Dict[str, Any]] = Field(None, description="Voice config override")
    scene_appearances: Optional[List[Dict[str, Any]]] = Field(None, description="Scene appearances")
    first_appearance_scene: Optional[int] = Field(None, description="First appearance scene number")
    last_appearance_scene: Optional[int] = Field(None, description="Last appearance scene number")
...
class EpisodeCharacterCreate(EpisodeCharacterBase):
    ...
    virtual_ip_id: int = Field(..., description="VirtualIP ID (required for resource binding)")
...
class EpisodeCharacterWithResources(EpisodeCharacterResponse):
    ...
    display_name: str = Field(..., description="Display name (character_name or VirtualIP.name)")
