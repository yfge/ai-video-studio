...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=True,
            comment="Character name (overrides VirtualIP.name)",
        ),
        sa.Column(
            "role_type",
            sa.String(length=50),
            ...
            comment="Role type: temporary/guest/extra",
        ),
        sa.Column(
            "importance",
            sa.Integer(),
            nullable=True,
            server_default="1",
            comment="Importance level 1-5, default 1",
        ),
        # Override fields
        sa.Column(
            "personality",
            sa.Text(),
            nullable=True,
            comment="Personality (overrides VirtualIP)",
        ),
        sa.Column(
            "background",
            sa.Text(),
            nullable=True,
            comment="Background (overrides VirtualIP)",
        ),
        sa.Column(
            "appearance_override",
            sa.Text(),
            nullable=True,
            comment="Appearance supplement to VirtualIP.style_prompt",
        ),
        sa.Column(
        ...
        sa.Column(
            ...
            sa.Integer(),
            nullable=True,
            comment="First appearance scene number",
        ),
        sa.Column(
            "last_appearance_scene",
            sa.Integer(),
            nullable=True,
            comment="Last appearance scene number",
        ),
        sa.Column(
        ...
        sa.Column(
            "deleted_by", sa.Integer(), nullable=True, comment="User who deleted"
        ),
        sa.Column(
            "deleted_reason", sa.Text(), nullable=True, comment="Deletion reason"
        ),
        # Timestamps
        sa.Column(
            "created_at",
        ...
        sa.ForeignKeyConstraint(
            ["virtual_ip_id"], ["virtual_ips.id"], ondelete="RESTRICT"
        ),
    )
    ...
    op.create_index(
        op.f("ix_episode_characters_business_id"),
        "episode_characters",
        ["business_id"],
        unique=True,
    )
    op.create_index(
        op.f("ix_episode_characters_episode_business_id"),
        "episode_characters",
        ["episode_business_id"],
    )
    op.create_index(
        op.f("ix_episode_characters_virtual_ip_business_id"),
        "episode_characters",
        ["virtual_ip_business_id"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_episode_characters_business_id"), table_name="episode_characters"
    )
