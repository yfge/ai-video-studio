...
def setup_test_environment():
    ...
    # 设置测试数据库
    if not run_command("python -c \"from app.core.test_database import setup_test_database; setup_test_database()\"", "设置测试数据库"):
        return False
...
def lint_code():
    ...
    # 运行flake8
    if not run_command("flake8 app/ tests/ --max-line-length=88 --extend-ignore=E203,W503", "运行flake8检查"):
...
def main():
    ...
    parser.add_argument("command", nargs="?", default="all", 
                       choices=["all", "unit", "integration", "migration", "api", "model", 
                               "e2e", "coverage", "quick", "parallel", "lint", "clean", "setup"],
                       help="要运行的测试类型")
