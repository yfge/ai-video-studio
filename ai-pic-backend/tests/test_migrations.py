...
class TestMigrations:
    ...
    def test_migration_creates_correct_columns(self):
        ...
        expected_vip_columns = [
            'id', 'name', 'description', 'tags', 'background_story',
            'style_prompt', 'style_reference_images', 'default_avatar_url',
        ...
        expected_story_columns = [
            'id', 'title', 'genre', 'theme', 'target_audience', 'duration_minutes',
            'premise', 'synopsis', 'main_conflict', 'resolution', 'main_characters',
            'character_relationships', 'setting_time', 'setting_location',
            'world_building', 'generation_prompt', 'ai_model', 'generation_params',
            'status', 'is_public', 'tags', 'extra_metadata', 'created_at', 'updated_at'
        ]
    ...
    def test_migration_creates_foreign_keys(self):
        ...
        assert len(vip_image_fks) > 0, "No foreign keys found in virtual_ip_images table"
    ...
    def test_migration_sqlite_compatibility(self):
        ...
        try:
            ...
            # 查询用户
            queried_user = session.query(User).filter(User.username == "testuser").first()
    ...
    def test_migration_idempotent(self):
        ...
        expected_tables = [
            'users', 'virtual_ips', 'virtual_ip_images',
            'stories', 'episodes', 'scripts', 'story_characters',
            'script_templates', 'alembic_version'
        ]
    ...
    def test_migration_script_syntax(self):
        ...
        for revision in revisions:
            assert revision.revision is not None, f"Revision {revision} has no revision ID"
    ...
    def test_migration_json_columns(self):
        ...
        try:
            ...
            # 查询并验证JSON数据
            queried_ip = session.query(VirtualIP).filter(VirtualIP.name == "Test IP").first()
            assert queried_ip.tags == ["tag1", "tag2"]
            assert queried_ip.style_reference_images == ["http://example.com/image1.jpg"]
