...
def test_page_access():
    ...
    try:
        ...
        if "AI Video Studio" in driver.title or len(driver.find_elements(By.TAG_NAME, "body")) > 0:
        ...
        if len(username_inputs) > 0 and len(password_inputs) > 0 and len(login_buttons) > 0:
            # 尝试登录
            ...
            if current_url != f"{BASE_URL}/login" and "login" not in current_url:
                ...
                if "用户管理" in page_content or "admin" in page_content or len(driver.find_elements(By.TAG_NAME, "table")) > 0:
