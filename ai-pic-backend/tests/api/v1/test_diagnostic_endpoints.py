...
class TestDiagnosticEndpoints:
    ...
    def test_openai_test_endpoint_with_auth(self, client, auth_headers):
        """测试带认证的OpenAI测试端点"""
        with patch('app.services.diagnostic_service.diagnostic_service.test_openai_api') as mock_test:
    ...
    def test_oss_test_endpoint_with_auth(self, client, auth_headers):
        """测试OSS服务测试端点"""
        with patch('app.services.diagnostic_service.diagnostic_service.test_oss_service') as mock_test:
            mock_test.return_value = True
    ...
    def test_end_to_end_test_endpoint_with_auth(self, client, auth_headers):
        """测试端到端测试端点"""
        with patch('app.services.diagnostic_service.diagnostic_service.test_end_to_end_image_generation') as mock_test:
            mock_test.return_value = True
            
            response = client.post("/api/v1/diagnostic/end-to-end", headers=auth_headers)
    ...
    def test_full_diagnostic_endpoint_with_auth(self, client, auth_headers):
        """测试完整诊断端点"""
        with patch('app.services.diagnostic_service.diagnostic_service.run_full_diagnostic') as mock_diagnostic:
