...
def test_sqlite_crud_operations():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # 测试查询用户
            result = conn.execute(text("SELECT username FROM users WHERE username = 'testuser'"))
            ...
            # 测试查询虚拟IP
            result = conn.execute(text("SELECT name, tags FROM virtual_ips WHERE name = 'Test IP'"))
            vip = result.fetchone()
            ...
            # 测试查询故事
            result = conn.execute(text("SELECT title FROM stories WHERE title = 'Test Story'"))
...
def test_foreign_key_relationships():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # 获取虚拟IP的ID
            result = conn.execute(text("SELECT id FROM virtual_ips WHERE name = 'Test IP'"))
...
def test_json_columns():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # 查询JSON数据
            result = conn.execute(text("SELECT tags, style_reference_images FROM virtual_ips WHERE name = 'Test IP'"))
            ...
            # 查询故事JSON数据
            result = conn.execute(text("SELECT main_characters, character_relationships, generation_params FROM stories WHERE title = 'Test Story'"))
