...
from app.services.providers.base import ProviderConfig, AIResponse, AITaskType, AIModelType
...
async def test_keling_with_mock():
    ...
    mock_response_data = {
        ...
        "message": "Success",
        "data": {
            "images": ["https://mock-keling.com/generated-image-123.png"]
        }
    }
    ...
    try:
        ...
        print(f"   可灵AI状态: {'✅可用' if keling_status.get('enabled') else '❌不可用'}")
        print(f"   支持的模型: {[m['name'] for m in keling_status.get('available_models', [])]}")
        
        # 模拟成功的AI管理器调用
        with patch.object(ai_service.ai_manager.providers['keling'], 'generate_image') as mock_generate:
    ...
    try:
        ...
        with patch.object(ai_service, '_generate_with_keling_image') as mock_keling_gen:
            mock_keling_gen.return_value = "https://mock-keling.com/generated-image-789.png"
