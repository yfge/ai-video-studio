...
from tests.factories import (
    ...
    StoryCharacterFactory, ScriptTemplateFactory,
    setup_factories
)
...
class TestVirtualIPModel:
    ...
    def test_virtual_ip_json_fields(self, db_session: Session):
        ...
        tags = ["tag1", "tag2", "tag3"]
        reference_images = ["http://example.com/image1.jpg", "http://example.com/image2.jpg"]
        
        vip = VirtualIPFactory(
            tags=tags,
            style_reference_images=reference_images
        )
    ...
    def test_virtual_ip_cascade_delete(self, db_session: Session):
        ...
        # 检查图像也被删除
        assert db_session.query(VirtualIP).filter(VirtualIP.id == vip_id).first() is None
        assert db_session.query(VirtualIPImage).filter(VirtualIPImage.id == image_id).first() is None
...
class TestModelRelationships:
    ...
    def test_cascade_delete_story(self, db_session: Session):
        ...
        assert db_session.query(StoryCharacter).filter(StoryCharacter.id == character_id).first() is None
