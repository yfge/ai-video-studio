doubao\-seedream\-4.5、doubao\-seedream\-4.0 支持流式输出模式。当您调用图片生成API 并将 `stream` 设置为 `true` 时，服务器会在生成响应的过程中，通过 Server\-Sent Events（SSE）实时向客户端推送事件。本节内容介绍服务器会推送的各类事件。
<span id="ScH1WJFo"></span>
## image_generation.partial_succeeded
> 当前仅 doubao\-seedream\-4.5、doubao\-seedream\-4.0 支持流式响应。
