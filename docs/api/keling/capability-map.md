...
> 备注（本项目实测）：`POST /v1/images/generations` 在 `model_name=kling-v2-1` 且携带 `image` 时会返回 `The V2.1 model currently does not support image-to-image`；图生图请使用 `kling-v2` / `kling-v1`（`kling-v1-5` 需设置 `image_reference`）。
