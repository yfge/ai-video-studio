...
- P0：无限画布已有交互、保存恢复、动态节点、类型化端口与边、按图输入解析、
  Run Node、Run Downstream 和任务证据基础；当前缺口是 stale descendants、
...
- 类型化端口、边、输入映射、Ready 评估和 Run Downstream 已接入真实请求；
  当前图执行缺口是定义、边或选用输出变化后的 stale descendants 传播。
...
- [x] 让后端按图依赖计算 Ready、Run Node 和 Run Downstream；边变化必须改变
      真实 Skill 请求输入。
- [ ] 定义、边或 selected output 变化时计算 stale descendants，并阻止过期结果
      被静默当作当前输入继续执行。
