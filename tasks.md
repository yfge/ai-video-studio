...
- 剩余范围是候选再生成/分支、协作权限与活动历史、可复用领域模板，以及
  大规模画布的性能预算和虚拟化，不影响首个 image-to-video-to-Timeline
  垂直切片实现；发布前仍需在当前环境做一次 provider-backed 整链集中复验。
...
- [ ] 发布前在当前环境重新跑一轮统一的 provider-backed 图片候选→选图→视频
      候选→选片→Timeline 回填，集中记录 console、network、task、媒体和 DB
      lineage 证据，避免把分散切片证据当作一次完整回归。
...
- [ ] 增加基于既有候选的再生成与分支，并持久化 parent candidate lineage。
- [ ] 增加节点、候选、边和分区评论，以及 Viewer、Commenter、Editor、Approver
      权限与可审计活动历史。
- [ ] 增加不暴露 provider 实现图的领域模板和可复用子流程。
- [ ] 定义大规模画布性能预算，接入视口虚拟化和规模回归证据。
