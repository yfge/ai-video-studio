...
- [x] 验证：`pytest` + `npm run lint` + Chrome E2E（登录→故事→剧集/剧本→分镜主路径），并记录到 `agent_chats`（2026-02-22：`pytest` 1883 passed / 87 skipped，`npm run lint` 0 error（6 warnings），Chrome 验证 `/stories` → `/episodes/136/workspace` → `/episodes/136/storyboard`）
...
- [x] 验证：构造 1 个 >8s 的长段（如 12s），确认被拆为 2 段且最终拼接后总时长与原轴一致（已通过 API 测试验证）
...
- [ ] 后端：拆分 `ai-pic-backend/app/api/v1/endpoints/scripts_legacy.py`（当前 4462 行）到 `app/api/v1/endpoints/scripts/*`（按 storyboard/dialogue-audio/timeline 等域拆分，已迁出 dialogue-audio/audio-timeline/timeline-pipeline），并逐步下线 legacy 路由
