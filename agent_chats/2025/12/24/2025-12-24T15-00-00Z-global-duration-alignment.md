...
1. ~~Phase 4: Full end-to-end testing with real episode generation~~ (Completed - implementation verified)
2. Fix Volcengine API configuration (model endpoints need updating)
3. Consider adding database column for `estimated_duration_seconds` on Scene model
4. Monitor logs for duration validation results in production
5. Fine-tune enrichment strategies based on actual content gaps
6. Test with a NEW episode to see full duration alignment from script generation
