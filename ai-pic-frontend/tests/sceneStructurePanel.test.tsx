...
const baseScene = { id: 1, scene_number: '1', slug_line: 'INT. TEST - DAY', status: 'draft' }
const beat = { id: 10, order_index: 1, beat_summary: 'beat' }
const shot = { id: 20, shot_number: '1A', shot_type: 'WS' }

const withProvider = (ui: React.ReactNode) => <AlertModalProvider>{ui}</AlertModalProvider>
...
describe('SceneStructurePanel permissions and callbacks', () => {
  ...
  beforeEach(() => {
    ...
    ;(apiClient as any).getNormalizedSceneBeats = stubClient.getNormalizedSceneBeats
    ;(apiClient as any).getNormalizedSceneShots = stubClient.getNormalizedSceneShots
  ...
  it('blocks create when read-only and surfaces warning', async () => {
    ...
    const utils = render(
      withProvider(<SceneStructurePanel scriptId={123} canEdit={false} apiOverride={stubClient} />),
