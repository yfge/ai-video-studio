...
import { type AIModel, type StoryboardVideoGenerationOptions } from '@/utils/api';
...
const deriveSeedreamVideoDefaults = (model?: AIModel): SeedreamVideoDefaults => {
  const modelId = model?.model_id || model?.id || '';
  const raw = modelId.includes(':') ? modelId.split(':')[1] : modelId;
  const caps = model?.capabilities || [];

  const supportsEndFrame = caps.includes('image_to_video_start_end_frame') && !raw.includes('fast');
  const supportsCameraFixed = !raw.includes('lite');

  // 图生视频优先使用更稳的 480p/720p（1080p 在参考图场景常见限制）
...
export function StoryboardVideoModal({
  ...
  const startList = useMemo(() => {
    ...
    if (defaultStart && !unique.includes(defaultStart)) unique.unshift(defaultStart);
    return unique;
  ...
  useEffect(() => {
    ...
    setModelId('');
  }, [defaultDuration, defaultEnd, defaultPrompt, defaultStart, endList, open, startList]);
  ...
  const defaults = useMemo(() => deriveSeedreamVideoDefaults(selectedModel), [selectedModel]);

  useEffect(() => {
    if (!open) return;
    setResolution(prev =>
      defaults.resolutionOptions.includes(prev) ? prev : defaults.defaultResolution,
    );
    setRatio(prev => (defaults.ratioOptions.includes(prev) ? prev : defaults.defaultRatio));
  ...
  const durationClamped = Math.max(2, Math.min(12, Number.isFinite(duration) ? duration : 5));
      ...
      <div className="w-full max-w-5xl overflow-auto rounded-xl bg-white p-5 shadow-2xl max-h-full">
        ...
        <div className="mt-5 space-y-4">
          <ModelSelector
            ...
            label="视频模型"
            helperText="当前仅支持 Seedream 图生视频"
            cacheKey="video-seedream-i2v"
            autoSelectDefault
            onModelsLoaded={(models, def) => {
              setAvailableModels(models);
              const preferred =
                def ||
                models.find(m => m.model_id === 'volcengine:seedream-i2v-pro')
                  ?.model_id ||
                models.find(m => m.provider === 'volcengine' && (m.model_id || '').includes('seedream'))
                  ?.model_id ||
                '';
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
              />
              <p className="mt-1 text-xs text-gray-500">支持 2~12 秒</p>
            </div>
            <div>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">seed（可选）</label>
              <input
                type="number"
                value={seedInput}
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <div className="text-xs text-gray-500">
            参数：duration={durationClamped}s, fps=24, resolution={resolution}, ratio={ratio}
          </div>
            ...
            <button
              ...
              onClick={async () => {
                const options: StoryboardVideoGenerationOptions = {
                  ...
                  camera_fixed: defaults.supportsCameraFixed ? cameraFixed : undefined,
                };
                await onSubmit({
                  start_image_url: startSelected,
                  end_image_url: defaults.supportsEndFrame ? endSelected || undefined : undefined,
                  options,
                });
              }}
