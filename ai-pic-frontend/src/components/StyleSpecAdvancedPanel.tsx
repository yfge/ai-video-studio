...
import type { StyleSpec } from '@/utils/api'
...
export function StyleSpecAdvancedPanel({
  ...
  const { schema, loading, error } = useStyleSchema({ enabled: fields.length > 0 })

  const dims = schema?.dimensions ?? {}

  const effectiveFields = useMemo(() => fields.filter(f => Boolean(f.key)), [fields])
    ...
    <div className="rounded border border-gray-200 bg-gray-50 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {helperText ? <div className="mt-1 text-[11px] text-gray-500">{helperText}</div> : null}
          {selectedCount > 0 ? (
            <div className="mt-1 text-[11px] text-gray-600">已选择 {selectedCount} 项</div>
          ) : null}
        </div>
      ...
      {expanded ? (
        <div className="mt-3">
          {loading ? (
            ...
            <div className="text-xs text-red-600">风格选项加载失败：{error}</div>
          ) : null}

          <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
            {effectiveFields.map(field => {
              const options = dims[field.key as string] ?? []
              const currentValue = (value?.[field.key] as string | undefined) ?? ''
              return (
                <div key={field.key} className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      if (!nextValue) {
                        delete (next as Record<string, unknown>)[field.key as string]
                      } else {
                        ;(next as Record<string, unknown>)[field.key as string] = nextValue
                      }
                      onChange(next)
                    }}
                  ...
                  {field.helperText ? (
                    <div className="text-[11px] text-gray-500">{field.helperText}</div>
                  ) : null}
                </div>
              )
            })}
