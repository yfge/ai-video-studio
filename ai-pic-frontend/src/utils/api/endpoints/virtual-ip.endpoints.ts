...
function virtualIPPath(ipIdOrBiz: number | string, suffix: string = ''): string {
 ...
 */
export async function getVirtualIP(id: number | string): Promise<ApiResponse<VirtualIP>> {
 ...
 */
export async function createVirtualIP(data: CreateVirtualIPRequest): Promise<ApiResponse<VirtualIP>> {
 ...
 * Delete a virtual IP.
 */
export async function deleteVirtualIP(id: number | string): Promise<ApiResponse<void>> {
...
export async function generateAIContent(
  ...
  return httpClient<VirtualIPAIGenerationResponse>('/api/v1/virtual-ips/generate-ai-content', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}
