...
import { styleAPI, type ApiResponse, type StyleSchemaResponse } from '@/utils/api'
...
export function useStyleSchema(options: UseStyleSchemaOptions = {}) {
  ...
  const cached = useMemo(() => schemaCache.get(effectiveKey) ?? null, [effectiveKey])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : '获取风格 schema 失败'
  ...
  useEffect(() => {
    ...
    if (schemaCache.has(effectiveKey)) {
      setState({ schema: schemaCache.get(effectiveKey) ?? null, loading: false, error: null })
      return
    }
    void load()
