...
export function useVirtualIPCreateForm({ showAlert, onCreated }: UseVirtualIPCreateFormOptions) {
  const [showCreateForm, setShowCreateForm] = useState(false)
  const [aiGenerating, setAiGenerating] = useState(false)
  const [aiBrief, setAiBrief] = useState('')
  const [formState, setFormState] = useState<VirtualIPCreateFormState>(createEmptyForm)
  ...
  const addTag = useCallback((tag: string) => {
    ...
    setFormState((prev) =>
      prev.tags.includes(cleaned) ? prev : { ...prev, tags: [...prev.tags, cleaned] }
    )
  }, [])
