...
export function useVoicePreview({
  ...
  const configSignature = useMemo(
    () =>
      `${voiceConfig.provider || ''}|${voiceConfig.model || ''}|${voiceConfig.voice_id || ''}|${
        voiceConfig.voice_type || ''
      }`,
    [voiceConfig.model, voiceConfig.provider, voiceConfig.voice_id, voiceConfig.voice_type],
  )
  ...
  const setPreviewUrls = useCallback((audioUrl: string | null, sourceUrl: string | null) => {
    setPreviewAudioUrl((prev) => {
      revokeIfBlob(prev)
      return audioUrl
    })
  ...
  const handlePreviewVoice = useCallback(async () => {
    const fallbackProvider = voiceConfig.provider || voiceEnums?.providers?.[0]?.value
    const fallbackModel =
      voiceConfig.model || voiceEnums?.defaults?.tts_model || voiceEnums?.tts_models?.[0]?.value
    const fallbackVoiceId =
      voiceConfig.voice_id || voiceEnums?.defaults?.voice_id || voiceOptions[0]?.value
